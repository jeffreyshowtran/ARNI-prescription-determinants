...
impmethod['GDMT_ACEIARB'] <- "2l.glm.bin" 
impmethod['GDMT_ARNI'] <- "" 
impmethod['GDMT_MRA'] <- "2l.binary"

impmethod['GDMT_BBcontra'] <- "2l.glm.bin"
...
impmethod['inhospRx_ACEIARB'] <- "2l.glm.bin"
...
impmethod['zip_designation'] <- ""
impmethod['population_total'] <- "2l.glm.norm"
impmethod['distress_score'] <- "2l.glm.norm"
...
start_time <- Sys.time() 
test1 <- mice(df, m=1, predictorMatrix=pm, method=impmethod,
                maxit=2, seed=88, cluster_var=cluster_var, 
                nAGQ=0, verbose=2,
                control1=glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=2e6)),
                control2=lmerControl(optimizer='nloptwrap', optCtrl=list(maxfun=2e6)))
end_time <- Sys.time() 
end_time - start_time 
#runtime start  1119 -> 1324 (takes a bit over 2hrs per iteration)

start_time <- Sys.time() 
test2 <- mice.mids(test1, maxit=2, print=F)
end_time <- Sys.time() 
end_time - start_time 
...
start_time <- Sys.time() 
print(start_time)
imprun <- parlmice(df, n.core=16, n.imp.core = 1, cl.type="FORK",
                   maxit=2, predictorMatrix=pm, method=impmethod,
...
write.mice.imputation(mi.res=imprun, name="impdir", mids2spss = FALSE)
