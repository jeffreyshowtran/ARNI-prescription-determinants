...
#m0 <- glmer(GDMT_ARNI ~ 1 + (1|SITE_ID), data = df.run, family=binomial)
#icc(m0)
#m0 <- lmer(distress_score ~ 1 + (1|SITE_ID), data = df.run)
#icc(m0)
#rm(m0)

#m1 <- nnet::multinom(insurance ~ 1 + (1|SITE_ID), data=df.run)
...
impmethod <- character(ncol(df.run))
names(impmethod) <- colnames(df.run)
...
pm <- make.predictorMatrix(df.run)
...
# imprun <- mice(df.run, m=1, predictorMatrix=pm, method=impmethod,
#                 maxit=1, seed=88, cluster_var=cluster_var, visitSequence=vis,
#                 nAGQ=0, verbose=2,
...
imprun <- parlmice(df.run, cluster.seed=88, n.core=3, cl.type="FORK",
                     ...
                     maxit=2, cluster_var=cluster_var, visitSequence=vis,
                     nAGQ=0, n.imp.core = 2,
                     control1=glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=2e5), calc.derivs=TRUE),
                     control2=lmerControl(optimizer='nloptwrap', optCtrl=list(maxfun=2e5)))
end_time <- Sys.time() 
